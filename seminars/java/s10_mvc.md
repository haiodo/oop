...
Для реализации связи между моделью и представлением используем `PropertyChangeListener` (рекомендуется с Java 9+, `java.util.Observable` устарел):
...
public class UserView {
  public UserView(UserModel model) {
    model.addListener(evt -> System.out.println("Имя изменилось: " + evt.getNewValue()));
  }
}
```
